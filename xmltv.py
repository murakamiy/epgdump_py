...
def create_xml(channel_id, service, events, filename, pretty_print):
    channel_el = create_channel(channel_id, service)
...
def create_channel(channel_id, service):
    attr = {'id':channel_id}
    channel_el = Element('channel', attr)
    attr = {'lang':'ja'}

    display_el = Element('display-name', attr)
    display_el.text = get_text(service.descriptors[0].service_name)
    ...
    display_el = Element('display-name', attr)
    display_el.text = channel_id + ' ' + get_text(service.descriptors[0].service_name)
    channel_el.append(display_el)
